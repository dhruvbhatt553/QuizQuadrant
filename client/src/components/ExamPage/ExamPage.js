...
export default function ExamPage() {
    ...
    useEffect(() => {
        const getExamData = async () => {
            let data = null;
            if (!isMockTest) {
                data = await fetchExamData(examId);
            } else {
...
return (<>
        {examData ? (!examFinish && (<div className='w-full h-screen absolute top-0 prevent-select'>
                ...
                </div>)) : (<div className='w-full h-full flex items-center grid justify-items-stretch'>
